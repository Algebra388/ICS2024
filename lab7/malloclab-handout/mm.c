...
#define PACK_SUM(size, pre_alloc, alloc)  ((size) | (pre_alloc << 1) | (alloc)) 
...
#define GET_PRE_ALLOC(p) ((GET(p) >> 1) & 1)               
...
#define PUT_PTR(ptr, bp)    (*(unsigned*)(ptr) = (unsigned)((long)((char*)bp - (char*)heap_low)))
/*only for displacement*/
#define PREV_BLKP(bp)       (heap_low + GET(bp))
#define NEXT_BLKP(bp)       (heap_low + GET((bp) + WSIZE))

#define SET_ALLOC_PRE(p)     (GET(p) |= 0x2)
#define SET_FREE_PRE(p)    (GET(p) &= ~0x2)     
...
int mm_init(void) 
{
    ...
    block_start = (void*)((char *)heap_low + DSIZE);
...
void *mm_malloc(size_t size) 
{
    ...
    if (size <= DSIZE)                                          
        ...
        asize = DSIZE * ((size + (WSIZE) + (DSIZE-1)) / DSIZE); ///double word alignment
    ...
    if(get_number(asize) == S_BLOCK - 1) extendsize = asize;
...
void mm_free(void *bp)
{
    ...
    PUT(HDRP(bp), PACK_SUM(size, pre_alloc, 0));
    PUT(FTRP(bp), PACK_SUM(size, pre_alloc, 0));
    //place_ptr(bp);
    coalesce(bp);
    check(240);
}
...
{ 
    ...
    while(ptr < (char*)mem_heap_hi())
    {
        ...
        /*if(GET_PRE_ALLOC(ptr) != is_allocated)
        {
            debug_printf("Line[%d] : allocated is not consistent! pointer = %p\n", lineno, ptr);
        }*/
    ...
    /*if((GET(HDRP(ptr)) & 3) != 3)
    {
        debug_printf("Line[%d] : epilogue's error! pointer = %p\n", lineno, ptr);
    }*/
}
...
static inline void remove_ptr(char *bp)
{
    ...
    //fprintf(stderr, "%d\n", GET(ptr));
    ptr = GET(ptr) + heap_low;
    while(ptr != bp) 
    {
        ptr = NEXT_BLKP(ptr);
        //assert(ptr != mem_heap_lo());
    }
    //assert(ptr == bp);
    ...
    if(NEXT_BLKP(ptr) != heap_low) 
        PUT_PTR(PREV(NEXT_BLKP(ptr)), PREV_BLKP(ptr));
    else 
        PUT_PTR(PREV(NEXT_BLKP(ptr)), heap_low);
    //check(456);
    //check_list(503);
    return ;
}
...
static void *extend_heap(size_t words) 
{
    ...
    PUT(HDRP(bp), PACK_SUM(size, prev_alloc, 0));         /* Free block header */   
    PUT(FTRP(bp), PACK_SUM(size, prev_alloc, 0));         /* Free block footer */   
    PUT(HDRP(SIZE_NEXT_BLKP(bp)), PACK(0, 3)); /* New epilogue header */ 
    /// my code here
...
static void *coalesce(void *bp) 
{
    ...
    if (prev_alloc && next_alloc) {            /* Case 1 */
        SET_FREE_PRE(HDRP(next_block));
        place_ptr(bp);
        return bp;
    }

    else if (prev_alloc && !next_alloc) {      /* Case 2 */
        if(verbose) debug_printf("coalesce 2: bp = %p %x, next = %p %x\n", bp, GET_SIZE(HDRP(bp)),\
            next_block, GET_SIZE(HDRP(next_block)));
        remove_ptr(next_block);
        //remove_ptr(bp);
        size += GET_SIZE(HDRP(next_block));
        PUT(HDRP(bp), PACK_SUM(size, 1, 0));
        PUT(FTRP(bp), PACK_SUM(size, 1, 0));
        place_ptr(bp);
    }
        ...
        size_t pp_alloc = GET_PRE_ALLOC(HDRP(prev_block));
        PUT(FTRP(bp), PACK_SUM(size, pp_alloc, 0));
        PUT(HDRP(prev_block), PACK_SUM(size, pp_alloc, 0));
        SET_FREE_PRE(HDRP(next_block));
        ...
        size_t pp_alloc = GET_PRE_ALLOC(HDRP(prev_block));
        PUT(HDRP(prev_block), PACK_SUM(size, pp_alloc, 0));
        PUT(FTRP(next_block), PACK_SUM(size, pp_alloc, 0));
        place_ptr(prev_block);
        bp = prev_block;
    }
#ifdef NEXT_FIT
...
static inline void place(void *bp, size_t asize)
{
    ...
    remove_ptr(bp);
    size_t pp_alloc = GET_PRE_ALLOC(HDRP(bp));
    if ((csize - asize) >= (2*DSIZE)) { 
        PUT(HDRP(bp), PACK_SUM(asize, pp_alloc, 1));
        //PUT(FTRP(bp), PACK(asize, 1));
        bp = SIZE_NEXT_BLKP(bp);
        PUT(HDRP(bp), PACK_SUM(csize-asize, 1, 0));
        PUT(FTRP(bp), PACK_SUM(csize-asize, 1, 0));
        place_ptr(bp);
    }
    else { 
        PUT(HDRP(bp), PACK_SUM(csize, pp_alloc, 1));
        SET_ALLOC_PRE(HDRP(SIZE_NEXT_BLKP(bp)));
        //PUT(FTRP(bp), PACK(csize, 1));
        if(!GET_ALLOC(HDRP(SIZE_NEXT_BLKP(bp)))) SET_ALLOC_PRE(FTRP(SIZE_NEXT_BLKP(bp)));
    }
    check(567);
}
