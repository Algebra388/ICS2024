...
int rotateRight(int x, int n) {
  ...
  //printf("%d\n",x>>n);
  //printf("%d\n",((x^((x>>n)<<n))<<(32-n)));
  //printf("%d\n",x>>n|((x^((x>>n)<<n))<<(32-n))|(t<<(31-n)));
  return (x>>n)|((x^((x>>n)<<n))<<(32+(~n)+1))|(t<<(31+1+(~n)));
  //return 2;
}
/* 
...
int countTrailingZero(int x){
  int ret=(!(x&0x0000FFFF))<<4;
  //printf("%d\n",ret);
  ret=ret+((!(x&(0x000000FF<<ret)))<<3);
  //printf("%d\n",ret);
  ret=ret+((!(x&(0x0000000F<<ret)))<<2);
  //printf("%d\n",ret);
  //printf("%d\n",x&(0x00000004<<ret));
  ret=ret+((!(x&(0x00000003<<ret)))<<1);
  //printf("%d\n",ret);
  ret=ret+((!(x&(0x00000001<<ret)))<<0);
  //printf("%d\n",ret);
  ret=ret+(!x);
  return ret;
    return 2;
...
int satMul3(int x) {
  ...
  int t=(x<<1)+x;
  int ans1=0,ans2=0;
  overflow|=(tval>>31)^(x>>31);
  overflow|=(t>>31)^(x>>31);
  //printf("%d\n",overflow);
  ans1=~overflow&t;
  ans2=overflow&(overflow&~((x>>31)^(1<<31)));
    return ans1|ans2;
}
/* 
...
unsigned float_twice(unsigned uf) {
  ...
  unsigned expr=uf>>23&0x000000FF;
  unsigned value=uf&((1<<23)-1);
  unsigned tval=value<<1;
  //printf("%x %x %x\n",expr,value,tval);
  if(expr==255)return uf;
  else if(expr==0)
  {
      if(tval<(1<<23))return sign|expr<<23|tval;
      else return sign|1<<23|(tval-(1<<23));
  }
  else
  {
  //printf("%u %u %u %u\n",sign,expr,value,tval);
    expr++;
    if(expr<=254)return sign|expr<<23|value;
    else if(expr==255)return sign|255<<23;
  }
  return -1;
}
/* 
...
unsigned float_i2f(int x) {
  /*unsigned sign=x>>31;
  int expr=0,pp=(sign<<31);
  unsigned ans=pp;
  int temp=0;
  int val1=0;
  int i,p;
  unsigned xx=pp?~x+1u:x;
  unsigned tval=xx;
  if(x==0)return 0;
  while(tval>=2)expr++,tval>>=1;
  val1=1<<expr;
  xx-=val1;
  //printf("%u %d\n",xx,val1);
  //printf("expr=%d\n",expr);
...
int float64_f2i(unsigned uf1, unsigned uf2) {
  int expr=uf2>>20&0x7ff;
  unsigned sign=uf2>>31&1;
  unsigned value=(uf2&0xfffff)<<11;
  int answer=0;
  value|=(uf1>>21)&0x7ff;
  value|=0x80000000;
  expr-=1023;
  if(expr<0)return 0;
  if(expr>=31)return 0x80000000;
  value=value>>(31-expr);
  answer=value;
...
unsigned float_negpwr2(int x) {
    ...
    int maxv=127;
    int expr;
    if(x==0x80000000)return 0x7f800000;
    x=-x;
    if(x<-minv)return 0;
    if(x>maxv)return 0x7f800000;
    expr=x+127;
    //printf("%d\n",x);
    if(-126<=x&&x<=127)return expr<<23;
    else return 1<<(minv+x);
    return 2;
}
